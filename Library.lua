...
local BaseURL = "https://raw.githubusercontent.com/Synergay/obsidian-fork/main/"
...
local function parsetxt(t)
    ...
    local s = t:gsub("<(%w+)>", function(c)
        local m = colors[c:lower()]
        if m then
...
local Library = {
    ...
    Scheme = {
        ...
        AccentGradientEnd = Color3.fromRGB(200, 100, 255),
    },
...
do
    ...
    function Funcs:AddToggle(Idx, Info)
        ...
        local ToggleGrad = New("UIGradient", {
            Enabled = Toggle.Value,
            Parent = Switch,
        })
        Library:RegisterGradient(ToggleGrad)
        ...
        function Toggle:Display()
            ...
            ToggleGrad.Enabled = Toggle.Value and not Toggle.Disabled
            Switch.BackgroundColor3 = (Toggle.Value and not Toggle.Disabled) and Color3.new(1, 1, 1) or (Toggle.Value and Library.Scheme.AccentColor or Library.Scheme.MainColor)
            SwitchStroke.Color = Toggle.Value and Library.Scheme.AccentColor or Library.Scheme.OutlineColor

            Library.Registry[Switch].BackgroundColor3 = (Toggle.Value and not Toggle.Disabled) and "WhiteColor" or (Toggle.Value and "AccentColor" or "MainColor")
    ...
    function Funcs:AddSlider(Idx, Info)
        ...
        local SliderGrad = New("UIGradient", { Parent = Fill })
        Library:RegisterGradient(SliderGrad)
        ...
        function Slider:UpdateColors()
            ...
            SliderGrad.Enabled = not Slider.Disabled
            Fill.BackgroundColor3 = Slider.Disabled and Library.Scheme.OutlineColor or Color3.new(1, 1, 1)
            Library.Registry[Fill].BackgroundColor3 = Slider.Disabled and "OutlineColor" or "WhiteColor"
        end
    ...
    function Funcs:AddSubSection(idx, info)
        info = Library:Validate(info, {
            Text = "subsection",
            Open = true,
        })
        local gbox = self
        local parent_c = gbox.Container
        local holder = New("Frame", {
            AutomaticSize = Enum.AutomaticSize.Y,
            BackgroundTransparency = 1,
        ...
        local btn = New("TextButton", {
            ...
            Text = (info.Open and "[-] " or "[+] ") .. info.Text,
            TextSize = 13,
            TextXAlignment = Enum.TextXAlignment.Left,
            Parent = holder,
        })
        ...
        btn.MouseButton1Click:Connect(function()
            sub.Open = not sub.Open
            c.Visible = sub.Open
            btn.Text = (sub.Open and "[-] " or "[+] ") .. info.Text
            gbox:Resize()
        end)
        function sub:Resize()
            gbox:Resize()
...
local grads = {}
function Library:RegisterGradient(g)
    table.insert(grads, g)
    local s = Library.Scheme.AccentGradientStart or Color3.fromRGB(125, 85, 255)
    local e = Library.Scheme.AccentGradientEnd or Color3.fromRGB(200, 100, 255)
    g.Color = ColorSequence.new(s, e)
end
function Library:UpdateGradients()
    local s = Library.Scheme.AccentGradientStart or Color3.fromRGB(125, 85, 255)
    local e = Library.Scheme.AccentGradientEnd or Color3.fromRGB(200, 100, 255)
    local seq = ColorSequence.new(s, e)
    for i = #grads, 1, -1 do
        local g = grads[i]
...
function Library:CreateWindow(WindowInfo)
    ...
    do
        ...
        WindowTitle = New("TextLabel", {
            ...
            Text = parsetxt(WindowInfo.Title),
            TextSize = 20,
            Parent = TitleHolder,
        })
        ...
        table.insert(Library.Corners, New("UICorner", {
            CornerRadius = UDim.new(0, WindowInfo.CornerRadius),
            Parent = DetachedHeader,
        }))
        Library:AddOutline(DetachedHeader)
        Library:MakeDraggable(DetachedFrame, DetachedHeader)
    ...
    function Window:ChangeTitle(title)
        ...
        WindowTitle.Text = parsetxt(title)
    ...
    Window.SidebarDetached = false
    function Window:SetSidebarDetached(state)
        Window.SidebarDetached = state
        if state then
            Tabs.Parent = DetachedFrame
    ...
    function Window:SetSidebarWidth(Width)
        ...
        if Window.SidebarDetached then
            Container.Size = UDim2.new(1, 0, 1, -70)
        else
